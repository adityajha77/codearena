...
declare_id!("9s1cWuVPtRUHxcf2iV5JgButwwkqx2qXx8kpx2HfhbGh");
#[program]
pub mod code_arena {
    use super::*;
    pub fn initialize_pool(
        ...
        pool.beneficiary = ctx.accounts.beneficiary.key();
    ...
    pub fn apply_penalty(ctx: Context<ApplyPenalty>) -> Result<()> {
        ...
        if pool.is_solo {
            let penalty_amount = pool.base_stake_amount / 2;
            // Transfer directly by mutating lamport balances (no PDA seeds needed for intra-program transfers)
    ...
    pub fn claim_payout(ctx: Context<ClaimPayout>) -> Result<()> {
        ...
        if pool.is_solo {
            if participant.strike_count == 0 {
                payout_amount = pool.base_stake_amount;
            } else {
                payout_amount = pool.base_stake_amount / 2;
            }
        } else {
        ...
        // Transfer directly by mutating lamport balances (no PDA seeds needed)
